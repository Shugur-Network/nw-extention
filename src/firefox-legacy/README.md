# Nostr Web Browser - Firefox Edition

> Decentralized static websites on Nostr - Firefox-compatible version

This is the Firefox-compatible version of the Nostr Web Browser extension. It provides the same functionality as the Chrome version but uses Firefox's WebExtension APIs.

## Key Differences from Chrome Version

### Architecture

- **Single Background Script**: Uses `background.js` instead of service worker + offscreen document
- **WebSocket Support**: WebSockets run directly in background script (Firefox allows this)
- **API Compatibility**: Uses `browser.*` API with `chrome.*` polyfill for compatibility

### Files

- ✅ `background.js` - Unified background script (replaces `sw.js` + `offscreen.js`)
- ✅ `manifest.json` - Firefox-specific manifest with `browser_specific_settings`
- ❌ `sw.js` - Not used (Chrome service worker)
- ❌ `offscreen.js` / `offscreen.html` - Not used (Chrome-only API)
...
- [Firefox Add-ons](https://addons.mozilla.org) (search for "Nostr Web") - Under Review
...
1. **Clone repository:**
   ...
   cd nostr-web/extension-firefox
   ```
...
3. **Test the extension:**
   ...
   - Enter `nweb.shugur.com`
   - Site should load from Nostr relays
...
2. **Upload to Firefox Add-ons:**
   ...
   - See [SUBMISSION_CHECKLIST.md](./SUBMISSION_CHECKLIST.md) for detailed steps
   - Full guide: [docs/FIREFOX_ADDON_SUBMISSION.md](../docs/FIREFOX_ADDON_SUBMISSION.md)
...
- **Permissions:**
  - `storage` - Cache DNS records and events
  - `webNavigation` - Detect Nostr Web sites
  - `alarms` - Future cache cleanup
  - `https://dns.google/*` - DNS-over-HTTPS queries
  - `https://cloudflare-dns.com/*` - DNS-over-HTTPS queries
...
**Firefox Edition** - Built for Mozilla Firefox 🦊
