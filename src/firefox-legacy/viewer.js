...
window.addEventListener("message", (event) => {
  logger.debug("Message received in viewer", {
    cmd: event.data?.cmd,
    data: event.data,
  });
  ...
  if (event.data?.cmd === "sandboxReady") {
    ...
    logger.info("Navigation requested", { route, currentDomain });
...
function assembleHTML(bundle) {
  ...
  // NOTE: Navigation handler is now in sandbox.js, not injected inline
  // This matches Chrome's renderer.js approach where the handler
  // is attached AFTER document.write() completes
  ...
  // Inject JS into <body> (no navigation handler)
...
function initSandbox() {
  ...
  sandboxFrame.src = browserAPI.runtime.getURL("sandbox.html");
...
settingsBtn.addEventListener("click", () => {
  window.location.href = browserAPI.runtime.getURL("settings.html");
});
...
if (urlParam) {
  ...
  browserAPI.storage.local.get(["nweb_default_site"], (result) => {
